...
def canonical(coefs):
    ...
    while coefs[-1] == 0 and len(coefs) > 1:
        del coefs[-1]
    return tuple(coefs)
# That's it for the main poly function. Here are the other necessary functions:
...
#Here is a more detailed test suite:
...
#Now for the extra credit. We'll define poly as a class, and provide the various methods that make it work with arithmetic operators and the equality test, as well as function call. If p is a poly then we want any expression of the form "p + z" to do polynomial addition. In particular, if z is, say, the integer 1, we want to convert that to a poly; I use coerce_poly for that.
...
# For the second part of the extra credit, I didn't bother to write a parser; I just used the class we defined above:

def Poly(formula): 
    "Parse the formula using eval in an environment where x is a poly."
    return eval(formula, {'x': poly((0, 1))})
# Note you need a memo function for this; here's one:
