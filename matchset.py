...
def matchset(pattern, text):
    ...
    elif 'dot' == op: # matches any character in the text, the remainder is the set
        return set([text[1:]]) if text else null # has to be text in the first place, null otherwise
    elif 'oneof' == op: # if the character is equal to the characters in x
        return set([text[1:]]) if text.startswith(x) # if x is a tuple, this can happen
    elif 'eol' == op:
