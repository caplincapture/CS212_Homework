...
#
# define two functions. The first is do(action, state), 
...
#
# The second function defined, strategy(state), should return an 
...
def do(action, state):
    ...
    cards_left = cards.replace(card, '', 1) # gives cards with card taken away
    if action == 'gather':
...
def strategy(state):
    (score, yard, cards) = state
    if 'F' not in cards: # may as well gather the rest of the chickens if no foxes left
        return 'wait' 
    elif yard >= 3: # trial and error found this
        return 'gather'
    else:
        return 'wait'
